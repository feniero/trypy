...
def roll_returns(portafogli,anni,dati, tickers,pesi):
    componenti = dati[tickers].dropna()
    performance = (componenti.shift(-12*anni)/componenti)**(1/anni)-1
    performance_indice = pd.DataFrame( np.dot(performance,pesi), index=performance.index, columns=["return"] )
    portafogli["return"] = performance_indice.dropna()
    return (portafogli["return"])
